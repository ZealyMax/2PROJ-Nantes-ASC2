...
$(document).on('change', 'select', function () {
    if ($(this).children('option:selected').val() == "short") {
        $(this).parent(".question").children(".question-content").html("<input name=short[] placeholder='Réponse courte' READONLY>");
    }
    if ($(this).children('option:selected').val() == "long") {
        $(this).parent(".question").children(".question-content").html("<textarea name=long[] placeholder='Réponse longue' READONLY></textarea>");
    }
    if ($(this).children('option:selected').val() == "multiple") {
        $(this).parent(".question").children(".question-content").html("<input type=radio><input name=multiple[] placeholder=Option><br><button class=add-choice>Ajouter</button></div>");
    }
    if ($(this).children('option:selected').val() == "checkbox") {
        $(this).parent(".question").children(".question-content").html("<input name=checkbox[] type=checkbox><input placeholder=Option><br><button class=add-check>Ajouter</button></div>");
    }
    if ($(this).children('option:selected').val() == "list") {
        $(this).parent(".question").children(".question-content").html("<input name=list[] placeholder=Option><br><button class=add-list>Ajouter</button></div>");
    }
